...
import { Productlinks, Evnetlinks } from "../../static";
import { Formik, Field, Form } from "formik";
import { EventValidation, ProductValidation } from "../../static/validator";
...
function CreateEventPage() {
  ...
  const SaveProduct = (values) => {
    ...
    dispatch(CreateEventAction(values))
      .then(() => {
        toast.success("The Items Add Scueessfuly");
      })
      .catch((e) => {
        toast.error("Some Thing Went Worng Try Again Later");
  ...
  return !Loading && shop != null ? (
    ...
    <DashboardLayout>
      ...
      <div className=" bg-white rounded-md p-3">
        <h2 className="text-2xl capitalize "> create Events</h2>

        <div className="bg-white py-8 px-4  sm:rounded-lg sm:px-10">
          {console.log(shop?._id, "From Log")}
          <Formik
            initialValues={{
              name: "",
              category: "",
                ...
                <Form className="">
                  ...
                  >
                    <div className="mt-2 relative flex   items-center border-dashed border-[3px] border-indigo-600  h-[160px] w-full">
                      <span className="flex h-[100%] gap-3   w-[100%]  overflow-hidden">
                        {images &&
                          images.map((el, i) => (
                            <img
                              key={i}
